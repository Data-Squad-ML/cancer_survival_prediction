...
"""Treina Random Forest com logica alinhada ao notebook e salva relatorio TXT."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Treino de Random Forest com Grid Search, KFold e ROC-AUC."
    )
...
def get_scoring_and_auc_mode(y_train: pd.Series) -> tuple[str, str]:
    n_classes = int(y_train.nunique())
    if n_classes <= 2:
        return "roc_auc", "binary"
    return "roc_auc_ovr_weighted", "multiclass"
...
def run_training(
    ...
    scoring, _ = get_scoring_and_auc_mode(y_train)

    pipeline = Pipeline(
        steps=[
            ("imputer", SimpleImputer(strategy="median")),
            ("model", RandomForestClassifier(random_state=random_state, n_jobs=rf_n_jobs)),
        ]
    )
...
def main() -> None:
    ...
    grid_search, scoring = run_training(
        x_train=x_train,
        y_train=y_train,
        w_train=w_train,
        cv=args.cv,
    ...
    _, auc_mode = get_scoring_and_auc_mode(y_train)
