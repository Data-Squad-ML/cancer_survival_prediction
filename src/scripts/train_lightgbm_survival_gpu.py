...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--shap-csv",
        type=Path,
        default=Path("results/lightgbm_survival_shap_global.csv"),
    )
    parser.add_argument(
        "--shap-plot",
        type=Path,
        default=Path("results/lightgbm_survival_shap_summary.png"),
    )
...
def build_survival_label_cox(time: np.ndarray, event: np.ndarray) -> np.ndarray:
    label = time.copy()
    label[~event] *= -1.0
    return label
...
class LGBMSurvivalCoxEstimator(BaseEstimator, RegressorMixin):
    """Wrapper para permitir y estruturado de survival no GridSearchCV.

    O LightGBM Cox recebe rótulo numérico com tempo positivo para evento
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.model_.fit(X, y_lgbm, **fit_kwargs)
        return self
...
def select_lgbm_mode(
    ...
    y_sample = y_train_lgbm[:sample_size]
    w_sample = w_train.iloc[:sample_size] if w_train is not None else None

    try:
        model = make_lgbm_model(random_state=random_state, lgbm_n_jobs=lgbm_n_jobs, mode="gpu")
...
def compute_and_save_global_shap(
    ...
    lgbm_model = wrapped_model.model_

    # SHAP no LightGBM deve usar a mesma representacao de entrada usada no treino.
...
def save_results(
    ...
    report_lines = [
        ...
        f"lgbm_n_jobs (modelo): {lgbm_n_jobs}",
        ...
        "",
        "--- Top 20 Feature Importance (gain do LightGBM) ---",
        lgbm_gain_df.head(20).to_string(index=False),
        "",
...
def main() -> None:
    ...
    grid_search, lgbm_mode = run_training(
        x_train=x_train,
    ...
    lgbm_gain_df, perm_df = compute_feature_importance_tables(
    ...
    print(f"Treinamento LightGBM Survival concluido. Resultados salvos em: {args.output}")
